...
class Contourlet_edge_exact(nn.Module):
    """Pytorch module for a resnet encoder
    """

    def __init__(self, outch):
        super(Contourlet_edge_exact, self).__init__()
        self.wt = ContourDec(2)
        self.iwt = ContourRec()
        self.edge_conv = nn.Sequential(nn.Conv2d(outch, outch * 2, 3, 2, 1, bias=False),
                                       nn.BatchNorm2d(outch * 2),
                                       nn.LeakyReLU()
                                       )
        self.conv1 = nn.Sequential(nn.Conv2d(outch, outch, 3, 2, 1, bias=False),
                                   nn.BatchNorm2d(outch),
                                   nn.LeakyReLU()
                                   )
        self.conv2 = nn.Sequential(nn.Conv2d(outch * 2, outch * 2, kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.BatchNorm2d(outch * 2),
                                   nn.LeakyReLU()
                                   )
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        x_ll0, h0 = self.wt(x)
        xlo = torch.full((b, 1, int(h / 2), int(w / 2)), 0).to(x.device)
        edge = self.iwt([xlo, h0])
        # edge = F.interpolate(edge, size=[h, w], mode='bilinear', align_corners=True)
        edge = self.edge_conv(edge)
        x = self.conv1(x)
        x = torch.cat((x, torch.abs(x_ll0)), dim=1)
        x = self.conv2(x)
...
class Contourlet_edge_exact_v1(nn.Module):
    """Pytorch module for a resnet encoder
    """

    def __init__(self, outch):
        super(Contourlet_edge_exact_v1, self).__init__()
        self.wt = ContourDec(2)
        self.iwt = ContourRec()
        self.edge_conv = nn.Sequential(nn.Conv2d(outch, outch * 2, 3, 2, 1, bias=False),
                                       nn.BatchNorm2d(outch * 2),
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(outch, outch * 2, 3, 2, 1, bias=False),
                                   nn.BatchNorm2d(outch * 2),
                                   nn.LeakyReLU()
                                   )
        self.conv2 = nn.Sequential(nn.Conv2d(outch * 3, outch * 2, kernel_size=3, stride=1, padding=1, bias=False),
                                   nn.BatchNorm2d(outch * 2),
                                   nn.LeakyReLU()
                                   )
...
class Contourlet_edge_exact_v2(nn.Module):
    """Pytorch module for a resnet encoder
    """

    def __init__(self, inch, outch):
        super(Contourlet_edge_exact_v2, self).__init__()
        self.wt = ContourDec(3)
        self.iwt = ContourRec()
        # self.wave1_f = nn.Sequential(nn.Conv2d(outch, outch, 3, 1, 1, bias=False),
        #                              nn.BatchNorm2d(outch * 2),
        #                              nn.LeakyReLU()
        #                              )
        # self.wave1_b = nn.Sequential(nn.Conv2d(outch, outch, 3, 1, 1, bias=False),
        #                              nn.BatchNorm2d(outch * 2),
        #                              nn.LeakyReLU()
        #                              )
        self.edge_conv = nn.Sequential(nn.Conv2d(inch, outch, 3, 1, 1, bias=False),
                                       nn.BatchNorm2d(outch),
                                       nn.LeakyReLU()
                                       )
        # self.conv = nn.Sequential(nn.Conv2d(inch * 2, outch, 3, 1, 1, bias=False),
        #                           nn.BatchNorm2d(outch),
        #                           nn.LeakyReLU()
        #                           )
    ...
    def forward(self, x):
        ...
        # hb = self.wave1_b(h0)
        # hhh = []
        # hhh.append(hf[:, 0, :, :].unsqueeze(1))
        # hhh.append(hf[:, 1, :, :].unsqueeze(1))
        # hhh.append(hf[:, 2, :, :].unsqueeze(1))
        # hhh.append(hf[:, 3, :, :].unsqueeze(1))
        # hhh.append(hb[:, 0, :, :].unsqueeze(1))
        # hhh.append(hb[:, 1, :, :].unsqueeze(1))
        # hhh.append(hb[:, 2, :, :].unsqueeze(1))
        # hhh.append(hb[:, 3, :, :].unsqueeze(1))
        xlo = torch.full((b, 1, int(h / 2), int(w / 2)), 0).to(x.device)
        edge = self.iwt([xlo, h0])
        edge = F.interpolate(edge, size=[h, w], mode='bilinear', align_corners=True)
