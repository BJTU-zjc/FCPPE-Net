...
class SGM(nn.Module):
    def __init__(self, in_c):
        ...
        self.contourlet_conv1 = nn.Sequential(
            nn.Conv2d(in_c, in_c * 2, 3, 2, 1, bias=False),
            nn.BatchNorm2d(in_c * 2),
            nn.ReLU(inplace=True))
        self.contourlet_conv2 = nn.Sequential(
            nn.Conv2d(in_c * 2, in_c * 4, 3, 2, 1, bias=False),
            nn.BatchNorm2d(in_c * 4),
            nn.ReLU(inplace=True))
        self.contourlet_conv3 = nn.Sequential(
            nn.Conv2d(in_c * 4, in_c * 8, 3, 2, 1, bias=False),
            nn.BatchNorm2d(in_c * 8),
            nn.ReLU(inplace=True))
        # out
        self.out_contourlet_conv1 = nn.Sequential(
            nn.Conv2d(in_c*4, in_c * 2, 3, 1, 1, bias=False),
            nn.BatchNorm2d(in_c * 2),
            nn.ReLU(inplace=True))
        self.out_contourlet_conv2 = nn.Sequential(
            nn.Conv2d(in_c * 8, in_c * 4, 3, 1, 1, bias=False),
            nn.BatchNorm2d(in_c * 4),
            nn.ReLU(inplace=True))
        self.out_contourlet_conv3 = nn.Sequential(
            nn.Conv2d(in_c * 16, in_c * 8, 3, 1, 1, bias=False),
            nn.BatchNorm2d(in_c * 8),
            nn.ReLU(inplace=True))
    ...
    def forward(self, ce_x1, ce_x2, ce_x3, ce_x4):
        ...
        edge_l1 = self.contourlet_conv1(edge1)
        ...
        edge_l2 = self.contourlet_conv2(edge_out_2)
        ...
        edge_l3 = self.contourlet_conv3(edge_out_3)
